...
public class JeiBridge implements IModPlugin {
    ...
    /** Called via reflection from GuiOverlay — safe because Class.forName only runs when JEI is loaded */
    public static void showRecipe(ItemStack stack, int key) {
        if (runtime == null) return;
        RecipeIngredientRole role = (key == org.lwjgl.glfw.GLFW.GLFW_KEY_R)
                ? RecipeIngredientRole.OUTPUT   // R key → how to craft this
                : RecipeIngredientRole.INPUT;   // U key → what this is used for
